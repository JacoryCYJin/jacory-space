...
const ActionLink = ({ href, icon, text, hoverText }) => {
  ...
  return (
    ...
    >
      <div
        className="absolute inset-0 bg-primary transform -translate-x-full group-hover:translate-x-0 transition-transform duration-500 ease-out"
        style={{ zIndex: 1 }}
      ></div>
      <div
      ...
      <div
        className="flex-1 px-6 py-4 flex items-center justify-between relative overflow-hidden"
        style={{ zIndex: 10 }}
      >
        <div className="relative">
          {/* 原始文字 */}
          <span className="text-primary font-bold text-sm uppercase tracking-wider transition-all duration-500 group-hover:opacity-0 group-hover:-translate-y-2 group-hover:text-background">
            {text}
          </span>
          {/* hover时显示的文字 */}
          <span className="absolute top-0 left-0 text-primary font-bold text-sm uppercase tracking-wider transition-all duration-500 opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 group-hover:text-background">
            {hoverText}
          </span>
        </div>
        <svg
          className="w-4 h-4 text-primary/70 group-hover:text-background hover:translate-x-0.5 hover:-translate-y-0.5 transition-all duration-500"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
      ...
      const setupScrollAnimations = () => {
        ...
        ScrollTrigger.create({
          ...
          onUpdate: (self) => {
            ...
            // 动态管理z-index层级，确保hover效果和J元素覆盖的协调工作
            if (progress < 0.05) {
              // 滚动开始前：按钮层级高于J元素，确保hover效果正常工作
              gsap.set(bottomLinksRef.current, { zIndex: 60 });
              gsap.set(jRef.current, { zIndex: 50 });
            } else {
              // 滚动开始后：J元素层级高于按钮，确保J能够覆盖按钮
              gsap.set(jRef.current, { zIndex: 60 });
              gsap.set(bottomLinksRef.current, { zIndex: 10 });
            }
