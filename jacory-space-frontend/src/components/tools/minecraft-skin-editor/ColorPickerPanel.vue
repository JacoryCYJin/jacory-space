<template>
  <section class="border border-line bg-card/95 backdrop-blur-sm" @keydown.esc="cancelHex">
    <div class="space-y-5 p-4">
      <div>
        <p class="tech mb-2 text-haze">{{ t('minecraftSkin.saturationLightness') }}</p>
      ...
      <label class="block">
        <span class="tech mb-2 block text-haze">{{ t('minecraftSkin.hexCode') }}</span>
      ...
      <div v-if="recentColors.length" class="border-t border-line pt-5">
        <p class="tech mb-2 text-haze">{{ t('minecraftSkin.recentColors') }}</p>
...
const ColorRange = defineComponent({
  ...
  setup(props, { emit }) {
    return () => h('label', { class: 'block' }, [
      h('span', { class: 'tech mb-2 block text-haze' }, props.label),
