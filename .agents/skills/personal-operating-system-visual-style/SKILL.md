...
* Shared tokens live in `app/globals.css`; shared primitives live in `components/` (`Reveal`, `PageFrame`, `SiteChrome`, `SectionHeading`, `SiteFooter`).

## Trigger

* **Always loaded.** This is a foundational style rule and must inform every design or UI decision.
* Path scope: all UI files — `app/**/*.tsx`, `components/**/*.tsx`, and `app/globals.css`.
...
* Color tokens are defined in `oklch` in `:root`; the hex values below are the canonical reference equivalents.
...
* Always reference semantic tokens (`bg-background`, `text-foreground`, `text-muted-foreground`, `text-blue`, `border-border`/`hairline`). Never hardcode raw colors in components.
...
* Verify in the browser (screenshot at 1440px and mobile) that the surface reads as an ordered cool archive, not a SaaS panel.
