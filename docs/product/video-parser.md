...
Video Parser 现在属于独立的 Media Parser 桌面软件。Jacory Space 网站只保留软件介绍页和下载入口；具体解析、下载、Cookie、转写和大纲生成能力都在 `../media-parser/` 仓库维护。

这份文档只描述用户能看到的产品流程，不展开具体代码实现。
...
它不是云端网盘，也不是公开视频库。当前设计更偏向个人本地工具：桌面端负责输入、展示和操作，Python media-core 负责调用解析能力、处理 Cookie、执行下载和保存文件。
...
默认更推荐浏览器模式。它让本地 media-core 从本机浏览器读取 Cookie，减少手动复制 Cookie 的成本。手动模式适合浏览器读取失败，或者需要为某个平台单独维护 Cookie 的情况。

## 当前边界

- 下载发生在运行桌面软件的本机，不是网站前端直接执行下载。
- 本地 media-core 需要可用的 `yt-dlp`，视频合并场景建议安装 `ffmpeg`。
